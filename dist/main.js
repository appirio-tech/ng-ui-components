...
$templateCache.put("views/image-viewer-header.directive.html","<main class=\"flex column middle light-bg\"><div class=\"header flex middle space-between\"><div class=\"user flex middle\"><a href=\"{{vm.generateProfileUrl(vm.handle)}}\" target=\"_blank\"><avatar avatar-url=\"{{ vm.avatar }}\"></avatar></a><a href=\"{{vm.generateProfileUrl(vm.handle)}}\" target=\"_blank\"><p class=\"name\">{{ vm.handle }}</p></a></div><div class=\"icons\"><button ng-if=\"vm.downloadAllowed\" class=\"clean\"><a href=\"{{ vm.file.url }}\" target=\"_blank\"><div class=\"icon download\"></div></a></button><button ng-click=\"vm.toggleComments()\" ng-if=\"vm.commentsAllowed\" class=\"clean\"><div class=\"icon bubble\"></div></button></div></div><hr/></main>");
